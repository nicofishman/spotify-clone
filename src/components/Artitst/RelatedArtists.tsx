  ...
  return relatedArtists?.artists && relatedArtists?.artists.length > 0 ? (
    <section>
      <h2 className='flex justify-between'>
        ...
        <Link className='hidden sm:block' href={`/artist/${artistId}/related`}>
      ...
      <CardSquareGrid isLoading={isLoading}>
        {({ columnCount }) =>
          relatedArtists?.artists.slice(0, columnCount).map((artist) => {
            return <RelatedArtistCardSquare key={artist.id} artist={artist} />;
          })
        }
      </CardSquareGrid>
    </section>
